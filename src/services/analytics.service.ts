...
export class AnalyticsService {
  ...
  private static calculateStats(jobs: any[]) {
    ...
    const formatRate = (count: number, total: number) => total > 0 ? (count / total * 100).toFixed(1) + '%' : '0.0%';
    ...
    const interviewedJobs = jobs.filter(j => {
      ...
      if (interviewStatuses.includes(j.status)) return true;
      if (j.history && j.history.some((h: any) => interviewStatuses.includes(h.status))) return true;
      return false;
    }).length;
    ...
    const idrSalaries = idrJobs.map(j => parseSalary(j.salary)).filter((s): s is number => s !== null);
    const usdSalaries = usdJobs.map(j => parseSalary(j.salary)).filter((s): s is number => s !== null);

    const getSalaryStats = (salaries: number[], format: (v: number) => string) => {
      if (salaries.length === 0) return { lowest: 'N/A', average: 'N/A', highest: 'N/A' };
      return {
        lowest: format(Math.min(...salaries)),
        average: format(Math.round(salaries.reduce((a, b: number) => a + b, 0) / salaries.length)),
        highest: format(Math.max(...salaries))
      };
    };

    // Job Fit Percentage
    const jfpData = jobs.map(j => j.jobFitPercentage).filter((v): v is number => v !== null);
    const getJfpStats = (data: number[]) => {
      if (data.length === 0) return { median: 'N/A', average: 'N/A', lowest: 'N/A', highest: 'N/A', basedOn: 0 };
      ...
      return {
        ...
        average: `${Math.round(data.reduce((a, b: number) => a + b, 0) / data.length)}%`,
    ...
    const stageTime: Record<string, { totalDays: number, count: number }> = {};
