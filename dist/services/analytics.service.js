...
export class AnalyticsService {
    static async getUserStats(userId) {
        ...
        const interviewedCount = jobs.filter(j => ['hr_interview', 'technical_interview', 'additional_interview'].includes(j.status)).length;
        ...
        const getDaysInPipeline = (job) => {
            if (!job.history || job.history.length < 2)
                return 0;
        ...
        const avgDaysInPipeline = Math.round(jobs.reduce((acc, j) => acc + getDaysInPipeline(j), 0) / totalJobs);
        return {
            all: totalJobs,
